...
public class DocServiceImpl implements DocService {
    ...
    public void review(long userId ,long cid , String tab ,int isPass){
		...
		try {
			c = Class.forName("com.jxufe.ctdms.reflect.SubmitTab"+captureName(tab));
	...
	public void saveDoc(long userId , UploadRecord doc  ) throws IOException{
		...
		SubmitTab submitTab = null ;
		try {
			c = Class.forName("com.jxufe.ctdms.reflect.SubmitTab"+captureName(doc.getTab()));
			submitTab = (SubmitTab) c.newInstance();
		} catch (ClassNotFoundException | InstantiationException | IllegalAccessException e) { 
			e.printStackTrace();
			return ;
		...
		//设置路径并新建文件夹
		String path = doc.getPath() +"/学期-" + TermServiceImpl.getNowTerm().getTermName() +"/" ;
