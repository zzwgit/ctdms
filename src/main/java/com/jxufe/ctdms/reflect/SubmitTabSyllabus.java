...
public class SubmitTabSyllabus extends SubmitTab {
	...
	public List<DocDto> getDocDtos(Object obj, User user) {
		...
		List<CourseTeacherTime> ctts = getSingeCourseTeacherTimes(dao.findByUser(user));
	...
	private List<CourseTeacherTime> getSingeCourseTeacherTimes(List<CourseTeacherTime> ctts) {
		List<CourseTeacherTime> c = new ArrayList<>();
		for (CourseTeacherTime ctt : ctts) {
			boolean isSinge = true;
			for (CourseTeacherTime iC : c) {
				if(iC.getCourse().getCourseCode().equals(ctt.getCourse().getCourseCode())){
					isSinge = false;
					break;
				}
			}
